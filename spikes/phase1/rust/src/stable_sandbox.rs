...
use crate::sandbox::SandboxProbeResult;
use serde::Serialize;
...
pub fn run_stable_probe(
    ...
    let result = loop {
        if response_path.exists() {
            ...
            if !bytes.is_empty() {
                if let Ok(parsed) = serde_json::from_slice::<SandboxProbeResult>(&bytes) {
                    break Ok(parsed);
                }
            }
        }
...
#[derive(Debug, Clone)]
pub struct BrokerEgressPolicy {
    pub allowed_targets: Vec<String>,
    pub timeout_ms: u64,
}

#[derive(Debug, Clone, Serialize)]
pub struct BrokerEgressEvidence {
    pub target: String,
...
pub fn broker_egress_connect(
    target: &str,
    policy: &BrokerEgressPolicy,
) -> Result<BrokerEgressEvidence, AdapterError> {
    if !policy.allowed_targets.iter().any(|value| value == target) {
        return Err(AdapterError::new(
            "STABLE_SANDBOX_EGRESS_DENIED",
            format!("egress target {target} is not allowlisted by RELAY"),
        ));
    }
    let address = target.parse::<SocketAddr>().map_err(|_| {
        AdapterError::new(
            "STABLE_SANDBOX_EGRESS_INVALID_TARGET",
            format!("egress target {target} is not a socket address"),
        )
    })?;
    let started = Instant::now();
    match TcpStream::connect_timeout(
        ...
        Err(error) => Ok(BrokerEgressEvidence {
            target: target.to_string(),
            allowed_by_policy: true,
            connect_ok: false,
            elapsed_ms: started.elapsed().as_secs_f64() * 1000.0,
