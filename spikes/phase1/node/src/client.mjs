...
export async function callHost(command, args = {}, timeoutMs = 3000) {
  const state = await readState().catch(error => {
    throw Object.assign(new Error("RELAY host state is unavailable."), { code: "HOST_UNAVAILABLE", cause: error });
  });
  return new Promise((resolve, reject) => {
    ...
    let greeted = false;
    const finish = value => { clearTimeout(timer); socket.end(); resolve(value); };
    const fail = (code, message) => { clearTimeout(timer); socket.destroy(); reject(Object.assign(new Error(message), { code })); };
    ...
    attachJsonLines(socket, message => {
      if (!greeted) {
        ...
        greeted = true;
        return sendJson(socket, { type: "command", request_id: requestId, schema_version: SCHEMA_VERSION, command, arguments: args });
      }
    ...
    socket.once("connect", () => sendJson(socket, {
      type: "hello", auth_token: state.auth_token, protocol_min: PROTOCOL_MIN, protocol_max: PROTOCOL_MAX,
      client: { name: "relay-cli", version: RELAY_VERSION }
    }));
  });
}
