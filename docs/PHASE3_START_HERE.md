...
Phase 3 promotes project discovery/indexing behavior into that production workspace. The generic two-project baseline, bounded change/dependency-edge foundation, provisional hint-update path, explicit content-verification recovery, bounded Windows watcher delivery, idle recovery scheduler, and versioned project configuration are accepted; see the seven Phase 3 slice evidence records. The 15,000-file single-host sample and NOT READY gate verdict are in `PHASE3_SCALE_BENCHMARK_EVIDENCE.md` and `PHASE3_CLOSURE_REVIEW.md`. Do not reopen Phase 1 stack selections or Phase 2 Core contracts without contradictory evidence.
...
> Take over DSI RELAY Phase 3 from `phase3/project-discovery`. Read root `AGENTS.md`, `docs/AGENTS.md`, `docs/PHASE3_START_HERE.md`, the seven Phase 3 slice evidence records, `PHASE3_SCALE_BENCHMARK_EVIDENCE.md`, and `PHASE3_CLOSURE_REVIEW.md` before editing. Phase 1 and Phase 2 are closed; the synthetic baseline, schema-5 change/dependency foundation, provisional targeted hint update, schema-6 continuity requirement, bounded Windows watcher and idle recovery scheduler, and schema-7 versioned project configuration are accepted. Preserve the Rust Core/command/authority/adapter contracts. Continue burst/scale recovery evidence, dependency extraction through a generic adapter boundary, and hardware-tier resource evidence. Revisit the NOT READY closure review only when those gates have passing proof. Keep project files authoritative and UEFN-specific behavior outside Core.
